...
contract BondingCurveHelper is Ownable{
    ...
    struct TokenStatus {
        ...
        uint256 endTime;  // init price of the token in ETH
    }
    ...
    function getMaxETH(address tokenAddress,address user, uint256 fundingEthAmount) external view returns (uint256 maxEth) {
        uint remainingEth = fundingEthAmount ;
        if(address(0) != tokenAddress){
            remainingEth = bondingCurve.calculateRemainingEthNeeded(tokenAddress,user);
        }
        (uint24 denominator ,uint256 percent ) = bondingCurve.getFeePercent();
        return remainingEth * denominator/ (denominator - percent);
    }
